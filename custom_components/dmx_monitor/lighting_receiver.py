...
class DmxNetworkReceiver:
    ...
    def __init__(self, interface: str, on_frame, on_timecode=None, on_poll_reply=None, multicast_universes=None,
        ...
        self.on_poll_reply = on_poll_reply
                ...
                if protocol == "ARTNET":
                    if self.on_timecode:
                        try:
                            ...
                            pass
                    if self.on_poll_reply:
                        try:
                            self.on_poll_reply(data, addr[0])
                        except Exception:
                            pass
                    parsed = parse_artnet_dmx(data)
                    if not parsed:
