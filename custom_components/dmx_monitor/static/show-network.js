...
<div class="footer"><button class="btn primary" id="modules">Modules & configuration</button><button class="btn" id="ha-config">Configuration générale HA</button><button class="btn" id="classic">Vue classique</button><button class="btn" id="timeline">Show Timeline</button><button class="btn" id="archive">Journal & backups</button></div>`;
  ...
  _renderModule(key){
    ...
    if(key==='video'){
      ...
      const sts=Object.values(this._hass?.states||{}).filter(x=>x.entity_id.includes('dmx_monitor')&&(`${x.entity_id} ${x.attributes?.friendly_name||''}`).toLowerCase().includes('projector'));
      const groups={}; for(const st of sts){const host=st.attributes?.host||st.attributes?.configuration_url||'projector'; (groups[host]??=[]).push(st)}
      const projectors=Object.entries(groups).map(([host,items])=>{const val=(needle)=>items.find(x=>(`${x.entity_id} ${x.attributes?.friendly_name||''}`).toLowerCase().includes(needle))?.state; const base=items[0]?.attributes||{}; return {host,manufacturer:base.manufacturer,model:base.model,power:val('power'),input:val('input source'),lamp:val('lamp'),temp:val('temperature'),errors:val('errors'),online:items.find(x=>x.entity_id.startsWith('binary_sensor.'))?.state};});
      const rows=projectors.length?projectors.map(x=>`<div class="card"><div class="title">${esc(x.manufacturer||'Projecteur')} ${esc(x.model||'')}</div><div class="big ${x.online==='on'?'ok':'off'}">${x.power||x.online||'—'}</div><div class="row"><span>Hôte</span><b>${esc(x.host)}</b></div><div class="row"><span>Entrée</span><b>${esc(x.input||'—')}</b></div><div class="row"><span>Lampe</span><b>${esc(x.lamp||'—')}</b></div><div class="row"><span>Température</span><b>${esc(x.temp||'—')}</b></div><div class="row"><span>Erreurs</span><b>${esc(x.errors||'—')}</b></div></div>`).join(''):'<div class="notice">Monitoring activé, mais aucun projecteur n’est configuré/découvert. PJLink expose état, entrée, heures lampe, température et erreurs lorsque le modèle les fournit.</div>';
      inner+=`<div class="card"><div class="title">VIDÉO / PROJECTEURS</div><div class="big ${enabled?'ok':'off'}">${enabled?'MONITORING ACTIVÉ':'DÉSACTIVÉ'}</div><div class="notice">Vue exploitation : online, power, source, lampe, température, erreurs et identité. Contrôle protégé par le verrou Show Network.</div></div><div class="module-grid">${rows}</div>`;
    }
