...
const SHOW_NETWORK_FRONTEND_VERSION = "0.15.2";
const SHOW_NETWORK_FRONTEND_BUILD = "cem3-readonly-20260915";
...
class ShowNetworkProDashboard extends HTMLElement {
  ...
  connectedCallback(){window.addEventListener('show-network-open-module',this._brandNav||(this._brandNav=(e)=>{const m=e.detail?.module;if(m)this._setView(`module:${m}`);}));this.addEventListener('focusout',this._focusOutHandler);this._ensureRegistry();this.render();this._cem3Timer=setInterval(()=>this._loadCem3(),5000);}
  disconnectedCallback(){clearInterval(this._cem3Timer);window.removeEventListener('show-network-open-module',this._brandNav);this.removeEventListener('focusout',this._focusOutHandler);if(this._raf){clearTimeout(this._raf);this._raf=null;}}
  ...
  async _loadCem3(){
    if(!this.isConnected||this._view!=='module:etc'||!this._hass?.connection||this._cem3Loading||Date.now()-this._cem3LastFetch<5000)return;
    this._cem3Loading=true;this._cem3LastFetch=Date.now();
    try{this._cem3=await this._hass.connection.sendMessagePromise({type:'dmx_monitor/cem3'});}
    catch(e){this._cem3={racks:[],total:0,online:0,error:String(e?.message||e)};}
    finally{this._cem3Loading=false;if(this.isConnected&&this._view==='module:etc'&&!this.matches?.(':focus-within'))this.render();}
  }
  ...
  _setView(view){this._uiDetails[this._view]=snCaptureDetails(this);this._uiScroll[this._view]=this.scrollTop;this._view=view;this._notice='';this._skipCapture=true;this.render();if(view==='module:video')this._loadVideoIp(true);if(view==='module:etc')this._loadCem3();}
...
<div class="footer"><button class="btn primary" id="modules">Modules & configuration</button><button class="btn" id="ha-config">Configuration générale HA</button><button class="btn" id="classic">Vue classique</button><button class="btn" id="timeline">Show Timeline</button><button class="btn" id="archive">Journal & backups</button></div>`;
  ...
  _modules(){return [
    ...
    ['gdtf','GDTF / Contrôle projecteurs','Profils GDTF, patch DMX et attributs HA mémorisables dans des scènes'],
    ['rdm','RDM / RDMnet','Découverte, identité, adresse, personnalité, capteurs et liaison avec GDTF'],
    ['osc','OSC / MIDI / PunchLight','Entrées, learn, mappings et sorties actives protégées'],
    ['show_control','Show Control','Cues HA + OSC + MIDI + scènes DMX, avec gate dédié'],
    ['rules','Rule Builder','Conditions DMX → actions Home Assistant, simulation et historique'],
    ['watchdog','Signal Watchdogs','Surveillance de présence/perte d’un flux attendu'],
    ['circuit_monitor','DMX Circuit Monitor','Retour d’état receive-only de plusieurs circuits DMX'],
    ['power','Power Manager','Boutons personnalisés et séquences DMX actives déphasées'],
    ...
    ['video','Vidéo / Projecteurs','Projecteurs, état, source, lampe, température et erreurs via PJLink ou adaptateur constructeur'],
    ...
    ['etc','ETC Sensor3 / CEM3','Capacités documentées ETC et état du transport live'],
  ...
  _renderModule(key){
    ...
    if(key==='rdm'){const e=this._state('rdm_devices_total','sensor.dmx_monitor_rdm_devices_total');const a=e?.attributes||{};const dev=a.rdm_devices||[];inner+=`<div class="card"><div class="title">RDM / RDMnet — DEVICE MANAGEMENT</div><div class="module-grid"><div class="card"><div class="title">Appareils</div><div class="big">${dev.length}</div></div><div class="card"><div class="title">En ligne</div><div class="big">${dev.filter(x=>x.online).length}</div></div><div class="card"><div class="title">Transports</div><div class="big">${esc((a.transports||[]).join(' / ')||'—')}</div></div></div><div class="notice">Découverte et GET sont read-only. Les SET RDM restent désactivés tant que <b>RDM writes</b> n'est pas armé dans les options et que Show Network n'est pas déverrouillé. Une UID peut être associée à un patch GDTF sans émettre de DMX.</div>${dev.map(x=>`<div class="card"><div class="title">${esc(x.device_label||x.model_description||x.uid)}</div><div class="row"><span>UID</span><b>${esc(x.uid)}</b></div><div class="row"><span>Transport</span><b>${esc(x.transport)} ${x.universe!=null?'· U'+x.universe:(x.scope?'· '+esc(x.scope):'')}</b></div><div class="row"><span>Fabricant / modèle</span><b>${esc(x.manufacturer_label||'—')} · ${esc(x.model_description||'—')}</b></div><div class="row"><span>Adresse DMX</span><b>${x.dmx_start_address??'—'} · footprint ${x.dmx_footprint??'—'}</b></div><div class="row"><span>Personnalité</span><b>${x.current_personality??'—'} / ${x.personality_count??'—'}</b></div><div class="row"><span>Capteurs / status</span><b>${(x.sensors||[]).length} / ${(x.status_messages||[]).length}</b></div><div class="row"><span>Fraîcheur</span><b>${x.online?'ONLINE':'STALE'} · ${x.age_s??'—'} s</b></div></div>`).join('')||'<div class="muted">Aucun responder RDM/RDMnet observé.</div>'}</div>`;}
    ...
    if(key==='etc'){const e=this._state('etc_cem3_racks_total','sensor.dmx_monitor_etc_cem3_racks_total');const live=this._cem3||e?.attributes||{};const racks=live.racks||[];const cat=this._state('etc_sensor_catalog','sensor.dmx_monitor_etc_sensor_catalog')?.attributes?.sensors||[];inner+=`<div class="card"><div class="title">ETC SENSOR3 / CEM3 · LIVE</div><div class="big">${live.online??0}/${live.total??0}</div><div class="muted">HTTP CEM3 en lecture seule · ${esc((live.source_ips||[]).join(', ')||live.source_ip||'route système')}</div><div class="notice">EXPERIMENTAL · Lecture seule : System et requêtes de lecture CEM3 validées. ${esc(live.discovery_status||'')} ${esc(live.error||'')}</div>${racks.map(r=>{const d=r.data||{},dm=r.dimmers||{};return `<div class="card"><div class="title">${esc(d.rack_name||r.host)} <span class="tag">${!r.online?'OFFLINE':r.fresh?'ONLINE':'STALE'}</span></div><div class="row"><span>IP</span><b>${esc(r.host||'—')}</b></div><div class="row"><span>Type / version</span><b>${esc(d.rack_type||'—')} · ${esc(d.software_version||'—')}</b></div><div class="row"><span>CPU</span><b>${d.cpu_temperature_c??'—'} °C</b></div><div class="row"><span>Secteur</span><b>${d.line_frequency_hz??'—'} Hz</b></div><div class="row"><span>Phases X/Y/Z</span><b>${d.phase_x_voltage_v??'—'} / ${d.phase_y_voltage_v??'—'} / ${d.phase_z_voltage_v??'—'} V</b></div><div class="row"><span>État / Panic</span><b>${esc(d.status||'—')} · ${esc(d.panic_state||'—')}</b></div><div class="row"><span>Erreurs</span><b>${r.errors_count??0}</b></div><div class="row"><span>Circuits visibles</span><b>${dm.circuits_total??'—'} · actifs ${dm.circuits_active??'—'}</b></div><div class="row"><span>Réponse</span><b>${r.response_ms??'—'} ms</b></div><div class="row"><span>Interface / dernière lecture</span><b>${esc(r.source_ip||'route système')} · ${r.age_seconds??'—'} s</b></div>
${r.last_error?`<div class="notice">${esc(r.last_error)}</div>`:''}
${Object.entries(r.section_errors||{}).map(([k,v])=>`<div class="notice">${esc(k)} : ${esc(v)}</div>`).join('')}
<div class="muted">Niveaux ${r.freshness?.levels?'frais':'anciens'} · propriétés ${r.freshness?.properties?'fraîches':'anciennes / indisponibles'} · espaces ${r.freshness?.spaces?'frais':'anciens / indisponibles'}</div>
${(r.spaces||[]).map(x=>`<div class="row"><span>Space ${esc(x.space)} · ${esc(x.name)}</span><b>Preset ${esc(x.active_preset)} · Sequence ${esc(x.active_sequence)}</b></div>`).join('')}
<details><summary>Circuits détaillés (${dm.circuits_total??0})</summary><div style="overflow:auto;max-height:480px"><table><thead><tr>${['Circuit','UDN','Space','Côté','Source','Niveau brut','Module','Contrôlable','Mode','Firing','Courbe','Seuil'].map(x=>`<th>${x}</th>`).join('')}</tr></thead><tbody>${(dm.circuits||[]).map(c=>`<tr>${['circuit','udn','space','side','wsource','level','module_type','controllable_module','control_mode','firing_mode','curve','threshold'].map(k=>`<td style="padding:5px 10px;white-space:nowrap">${esc(c[k]??'—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details>
</div>`}).join('')}<div class="title">CAPACITÉS ETC</div>${cat.slice(0,80).map(x=>`<div class="row"><span>${esc(x.label_fr||x.name||x.key||'—')}</span><b>${x.live_supported?'LIVE':'DOCUMENTÉ'}</b></div>`).join('')}</div>`;}
    ...
    if(key==='network'){const cfgState=this._state('show_network_config','sensor.dmx_monitor_show_network_config');const cfg=cfgState?.attributes||{};const yn=(k)=>cfgState?(cfg[k]===true?'Écouté':cfg[k]===false?'Désactivé':'Indisponible'):'État indisponible';const sw=this._state('switch_telemetry','sensor.dmx_monitor_switch_telemetry')?.attributes?.switches||[];inner+=`<div class="card"><div class="title">SWITCHES IDENTIFIÉS — TÉLÉMÉTRIE READ-ONLY</div>${sw.length?sw.map(x=>`<div class="row"><span>${esc(x.name||x.ip)} · ${esc(x.manufacturer||'—')}</span><span>${x.ports_up??0}/${x.interface_count??0} interfaces UP</span><span>${x.temperature_c??'—'} °C</span><span>${esc(x.ip||'—')}</span></div>`).join(''):'<div class="notice">Aucun switch n’a encore répondu avec une preuve suffisante. Luminex peut être reconnu par MAC enregistré, HTTP ou SNMP; la télémétrie de ports nécessite SNMP.</div>'}</div><div class="card"><div class="title">CONFIGURATION RÉSEAU SHOW CONTROL</div>${cfgState?'':`<div class="notice">Capteur de configuration indisponible : aucun état activé/désactivé n'est supposé.</div>`}<div class="row"><span>DMX / Art-Net / sACN</span><b>${cfg.interface_dmx??'—'}</b></div><div class="row"><span>grandMA3 / MA-Net3</span><b>${cfg.interface_ma??'—'}</b></div><div class="row"><span>Dante</span><b>${cfg.interface_dante??'—'}</b></div><div class="row"><span>PTP</span><b>${cfg.interface_ptp??'—'}</b></div><div class="row"><span>Audio AES67/ST2110</span><b>${cfg.interface_audio??'—'}</b></div><div class="row"><span>Art-Net</span><b>${yn('dmx_artnet_enabled')}</b></div><div class="row"><span>sACN</span><b>${yn('dmx_sacn_enabled')}</b></div><div class="row"><span>Univers DMX</span><b>${cfg.universes??'—'}</b></div><div class="row"><span>MA-Net3</span><b>${yn('ma_enabled')}</b></div><div class="footer"><button class="btn primary" id="network-config">Modifier les interfaces / protocoles</button></div></div>`;}
    ...
    const map={show_control:['show-control-panel'],dmx:['dmx-monitor-panel','enttec-panel'],zones:['dmx-ha-zones-panel','dmx-ha-mapping-panel'],osc:['control-sources-panel','osc-learn-panel','osc-mapping-panel','osc-output-panel','osc-source-profiles','punchlight-network-panel'],rules:['show-network-rule-builder'],watchdog:['signal-watchdog-panel'],circuit_monitor:['dmx-circuit-monitor-panel'],power:['power-manager-panel'],network:['show-network-topology-panel','show-network-discovery','show-network-fingerprint'],ma:['ma-inspector-panel'],inventory:['show-network-inventory'],builder:['show-network-ha-builder-panel'],brands:['show-network-brand-catalog'],reliability:['show-network-reliability-panel'],archive:['show-network-archive-panel']};
...
class ShowControlPanel extends HTMLElement {
  constructor(){super();this._hass=null;}
  ...
  _attrs(){
    ...
    return {cue:cueState?.attributes||{},midi:midiState?.attributes||{}};
  }
  ...
  render(){
    ...
    this.innerHTML=`<style>:host{display:block;background:#0b0d10;color:#eee;font-family:Inter,system-ui,sans-serif;padding:18px}.top{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px}.card{background:#15181c;border:1px solid #2c3239;border-radius:9px;padding:13px;margin-bottom:10px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px}.big{font-size:24px;font-weight:800}.muted{font-size:10px;color:#8d969f}.ok{color:#65dc99}.off{color:#f0b36a}.btn{background:#252a30;border:1px solid #3a4148;color:#eee;border-radius:6px;padding:8px 10px;cursor:pointer}.btn.active{background:#1d5c3a}.row{display:grid;grid-template-columns:1fr 1.4fr 1fr auto;gap:8px;padding:8px 0;border-top:1px solid #282d33;font-size:11px;align-items:center}</style>
      ...
      <div class="card"><b>Cues enregistrés</b>${cues.length?cues.map(c=>`<div class="row"><span>${c.name||c.cue_id}</span><span>${(c.actions||[]).length} action(s)</span><span>${c.enabled===false?'OFF':'READY'}</span><button class="btn" data-fire="${c.cue_id}">GO</button></div>`).join(''):'<div class="muted" style="margin-top:8px">Aucun cue. Utiliser le service upsert_show_control_cue pour créer un cue HA/OSC/MIDI/DMX.</div>'}</div>
