...
class ShowNetworkProDashboard extends HTMLElement {
  ...
  set hass(h){this._hass=h; this._ensureRegistry(); if(this._raf)return; const run=()=>{this._raf=null;if(this._view.startsWith('module:')){const key=this._view.slice(7);if(['audio','amplifiers','video','security','network','etc'].includes(key)&&!this.matches?.(':focus-within')){this.render();return;}this.querySelectorAll('#module-content > *').forEach(el=>{if(el.matches?.(':focus-within'))return;const open=snCaptureDetails(el),top=el.scrollTop;try{el.hass=this._hass}catch(e){}snRestoreDetails(el,open);el.scrollTop=top;});}else{this.render();}}; this._raf=setTimeout(run,750);}
