...
class MARemoteInventory:
    ...
    def snapshot(self) -> dict:
        ...
        for s in self.stations.values():
            if s.last_seen is not None and now - s.last_seen > 15: s.ma_net3_active = False
        rows = [s.snapshot() for s in self.stations.values()]; rows.sort(key=lambda x: x["ip"])
        sessions=[]
        for idx in sorted({s.session_index for s in self.stations.values() if s.session_index is not None}):
            ...
            sessions.append({"session_index": idx, "member_count": len(members), "members": [m["ip"] for m in members], "name": None, "location": None, "master": None, "evidence": "MA-Net3 multicast group"})
        return {
            "stations": rows, "station_count": len(rows), "live_stations": sum(1 for x in rows if x["state"] == "LIVE"),
