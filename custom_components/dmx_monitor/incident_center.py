...
class IncidentCenter:
    ...
    def build(self, recorder: dict, devices: dict | None = None) -> dict:
        ...
        for raw in (recorder or {}).get("incidents", [])[-12:]:
            ...
            incidents.append({"id":iid,"ts":raw.get("ts"),"severity":raw.get("severity","info"),"anchor":{"kind":raw.get("kind"),"event":raw.get("event")},"symptom_count":len(evs),"families":kinds,"device_ids":devs,"symptoms":symptoms[:12],"statement":"Événements observés dans la même fenêtre temporelle; aucune cause racine n'est déduite.",**row,"duration_s":duration})
        ...
        return {"state":"error" if counts.get("error") else "warning" if counts.get("warning") else "ok","incident_count":len(incidents),"active_count":status_counts.get("ACTIVE",0),"acknowledged_count":status_counts.get("ACKNOWLEDGED",0),"resolved_count":status_counts.get("RESOLVED",0),"counts":dict(counts),"status_counts":dict(status_counts),"incidents":incidents,"evidence_only":True,"root_cause_inferred":False}
