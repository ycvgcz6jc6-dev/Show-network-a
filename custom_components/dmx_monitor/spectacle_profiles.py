...
def _load() -> tuple[ShowManufacturerProfile, ...]:
    raw = load_yaml_catalog("spectacle_profiles.yaml", _validate)
    return tuple(ShowManufacturerProfile(x["key"],x["manufacturer"],tuple(x["domains"]),tuple(x["product_families"]),tuple(x["passive_protocols"]),tuple(x["diagnostics"]),tuple(x.get("control_paths",()))) for x in raw)
