...
<div class="row"><select data-rdm-link-field="${esc(x.uid)}" style="flex:1">${gdtfPatches.length?gdtfPatches.map(p=>`<option value="${esc(p.patch_id)}" ${p.rdm_uid===x.uid?'selected':''}>${esc(p.name)}${p.rdm_uid===x.uid?' (déjà lié)':''}</option>`).join(''):'<option value="">Aucun patch GDTF — crée-en un dans GDTF</option>'}</select><button class="btn" data-rdm-link="${esc(x.uid)}" ${gdtfPatches.length?'':'disabled'}>Lier au patch</button></div>
    ...
    if(key==='etc'){const e=this._state('etc_cem3_racks_total','sensor.dmx_monitor_etc_cem3_racks_total');const live=e?.attributes||{};const racks=live.racks||[];const cat=this._state('etc_sensor_catalog','sensor.dmx_monitor_etc_sensor_catalog')?.attributes?.sensors||[];inner+=`<div class="card"><div class="title">ETC SENSOR3 / CEM3 · LIVE</div><div class="big">${live.online??0}/${live.total??0}</div><div class="muted">HTTP CEM3 en lecture seule · ${esc(live.source_ip||'route système')}</div><div class="notice">Show Network utilise l’interface Web CEM3 en lecture seule : pages HTTP GET et requêtes de lecture fixes observées sur CEM3. Aucune modification de rack, niveau, preset ou configuration n’est envoyée.</div>${racks.map(r=>{const d=r.data||{},dm=r.dimmers||{};const circuitRows=(dm.circuits||[]).slice(0,200).map(c=>`<div class="row"><span>Esp.${esc(c.space??'—')}/Circ.${esc(c.circuit??'—')}</span><b>${esc(c.module_type||'—')} · ${esc(c.control_mode||'—')} · ${esc(c.curve||'—')}${c.level_percent!=null?` · ${c.level_percent}%`:''}</b></div>`).join('');return `<div class="card"><div class="title">${esc(d.rack_name||r.host)} <span class="tag">${r.online?'ONLINE':'OFFLINE'}</span></div><div class="row"><span>IP</span><b>${esc(r.host||'—')}</b></div><div class="row"><span>Type / version</span><b>${esc(d.rack_type||'—')} · ${esc(d.software_version||'—')}</b></div><div class="row"><span>Secteur</span><b>${d.line_frequency_hz??'—'} Hz</b></div><div class="row"><span>Phases X/Y/Z</span><b>${d.phase_x_voltage_v??'—'} / ${d.phase_y_voltage_v??'—'} / ${d.phase_z_voltage_v??'—'} V</b></div><div class="row"><span>État / Panic</span><b>${esc(d.status||'—')} · ${esc(d.panic_state||'—')}</b></div><div class="row"><span>Erreurs</span><b>${r.errors_count??0}</b></div><div class="row"><span>Circuits visibles</span><b>${dm.circuits_total??'—'} · actifs ${dm.circuits_active??'—'}</b></div><div class="row"><span>Réponse</span><b>${r.response_ms??'—'} ms</b></div>${r.last_error?`<div class="notice">${esc(r.last_error)}</div>`:''}${circuitRows?`<div class="title" style="margin-top:8px;font-size:12px">Configuration par circuit (module · mode · courbe)</div><div style="max-height:260px;overflow-y:auto">${circuitRows}</div>`:''}</div>`}).join('')}<div class="title">CAPACITÉS ETC</div>${cat.slice(0,80).map(x=>`<div class="row"><span>${esc(x.label_fr||x.name||x.key||'—')}</span><b>${x.live_supported?'LIVE':'DOCUMENTÉ'}</b></div>`).join('')}</div>`;}
